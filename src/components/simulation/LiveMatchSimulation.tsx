...
export default function LiveMatchSimulation() {
  ...
  useEffect(() => {
    ...
    if (isRunning && !isPaused && !isHalfTime) {
      interval = setInterval(() => {
        setCurrentMinute(prev => {
          const newMinute = prev + 1;
          
          // Half time at 45 seconds (45 minutes)
          if (newMinute === 45) {
            setIsHalfTime(true);
            setIsRunning(false);
            toast.info('Half Time! Click Resume to continue second half.');
            return newMinute;
          }
  ...
  const resumeMatch = () => {
    if (isHalfTime) {
      setIsHalfTime(false);
      toast.info('Second half started!');
    }
    setIsPaused(false);
    setIsRunning(true);
  };
            ...
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center space-x-4">
                ...
                <div className="flex items-center text-lg text-gray-600">
                  ...
                  {currentMinute}'
                  {isHalfTime && <span className="ml-2 text-orange-600 font-medium">HALF TIME</span>}
              ...
              <div className="flex space-x-2">
                ...
                {(isPaused || isHalfTime) && currentMinute < 90 && (
                  <button
                    onClick={resumeMatch}
                    className="bg-green-600 text-white px-4 py-2 rounded-lg hover:bg-green-700 transition-colors flex items-center"
                  >
                    <Play className="h-4 w-4 mr-2" />
                    {isHalfTime ? 'Start 2nd Half' : 'Resume'}
                  </button>
                )}
                ...
                {[...homePlayers, ...awayPlayers]
                  ...
                  <div key={player.player_id} className="relative">
                    <select
                      onChange={(e) => {
                        if (e.target.value) {
                          addMatchEvent(e.target.value as MatchEvent['type'], player.player_id);
                          e.target.value = '';
                        }
                      }}
